...
## This PRD’s Specific Purpose – Agent Assist Unibox Plugin (Phase 1)

This specific PRD defines **Agent Assist – an AI plugin embedded in the Unibox text editor**. The plugin connects to an LLM-backed service that is **strictly grounded in Knowledge Bases ingested via the Chatbot/NextIQ KB ingestion API**. Operations teams upload URLs and documents into this ingestion service so that Agent Assist can answer questions only from those curated KBs.

During live customer interactions, agents can:

- **Search and chat against those KBs** directly from Unibox to resolve issues faster.  
- Use inline tools to **rephrase, shorten, expand, translate, summarize, and suggest responses** for their drafted replies.

Phase 1 focuses on:

- Enforcing **hard OOS boundaries** – if an answer is not supported by the ingested KBs, the system must clearly indicate the question is out of scope instead of hallucinating.  
- Integrating with the existing **Chatbot/NextIQ KB ingestion APIs** and AI backend.  
- Delivering these capabilities with **minimal UI change** to Unibox, prioritizing backend, orchestration, and safeguards over visual redesign.
